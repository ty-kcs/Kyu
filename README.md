...
### 1. 疲れ傾向モデル (gpr_model.py)
- ガウス過程回帰 (GPR) による24時間疲労度予測
- **特徴量**: 時刻 (0.0 ~ 24.0)
- **ターゲット**: 疲労度 (1-7)
- **客観補正機能**: 主観と客観の乖離が大きい場合、客観値を重視
- **不確かさの活用**: 標準偏差が高い時間帯を特定し、データ不足の時間帯を通知。

### 2. 不調推定モデル (face_analyzer.py, predict_fatigue.py)
- OpenFaceのAction Units (AU) から客観的疲労度を算出
- **特徴量**: 顔画像から抽出したAU（顔面筋の動きの定量化指標）
- **アルゴリズム**: ランダムフォレストを使用し、疲労度を「低・中・高」の3クラスに分類
- **課題**: 極度の疲労状態のデータ不足により、分類精度向上の余地あり
...
## Google Colab の役割
Google Colab を活用することで、以下のタスクを効率的に実行できます。

1. **OpenFaceのセットアップ**:
   - OpenFaceのビルドやモデルのダウンロードを自動化。
   - 必要なライブラリや依存関係をインストール。

2. **顔画像の解析**:
   - Google Drive から顔画像を取得し、OpenFaceで特徴量を抽出。
   - 抽出した特徴量を基に疲労度を推定。

3. **Slack通知の自動化**:
   - 疲労度推定結果を基に、Slackにアドバイスメッセージを送信。

4. **データ収集と管理**:
   - Google スプレッドシートからアンケートデータを取得し、モデルに反映。
